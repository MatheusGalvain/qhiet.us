      ...
      <div style={{ border: '1px solid var(--faint)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '56px 1fr 160px 100px 90px 100px 100px', gap: 0, padding: '10px 20px', borderBottom: '1px solid var(--faint)', background: 'rgba(255,255,255,0.02)' }}>
          {['Nº', 'Título', 'Categorias', 'Acesso', 'Leitura', 'Status', ''].map(h => (
            <span key={h} style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 3, textTransform: 'uppercase', color: 'var(--muted)' }}>{h}</span>
          ))}
        </div>
        ...
        {list.map((t, i) => (
          ...
          >
            ...
            <div>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 1, color: 'var(--muted)' }}>/{t.slug}</span>
            </div>

            {/* Categories */}
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {(t.categories ?? []).slice(0, 2).map((cat: string) => (
                <span key={cat} style={{ fontFamily: 'var(--font-mono)', fontSize: 7, letterSpacing: 1, color: 'var(--muted)', border: '1px solid var(--faint)', padding: '2px 6px' }}>
            ...
            {/* Access */}
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, textTransform: 'uppercase', color: t.access === 'free' ? 'var(--muted)' : 'var(--red)', border: `1px solid ${t.access === 'free' ? 'var(--faint)' : 'var(--red-dim)'}`, padding: '3px 8px', display: 'inline-block' }}>
            ...
            {/* Status */}
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, textTransform: 'uppercase', color: t.status === 'published' ? 'var(--gold)' : 'var(--muted)', border: `1px solid ${t.status === 'published' ? 'var(--gold)' : 'var(--faint)'}`, padding: '3px 8px', opacity: t.status === 'published' ? 1 : 0.6, display: 'inline-block' }}>
              {t.status === 'published' ? 'publicado' : 'rascunho'}
            </span>

            {/* Actions */}
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <Link href={`/admin/transmissoes/${t.id}/editar`} style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, color: 'var(--muted)', textDecoration: 'none', textTransform: 'uppercase' }}>
                Editar
              </Link>
