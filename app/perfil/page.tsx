...
async function getData() {
  ...
  return { profile: profile as Profile, xpEvents: xpEvents ?? [], books: books ?? [] }
}

export default async function PerfilPage() {
  const { profile, xpEvents, books } = await getData()
      ...
      <aside className="profile-sidebar">
        ...
        {!profile.is_subscriber && (
          <div style={{ padding: '0 32px', marginBottom: 16, marginTop: 'auto' }} className="profile-identity-mobile-hide">
            <Link href="/membros" className="btn-primary" style={{ display: 'block', textAlign: 'center', fontSize: 9, padding: '12px' }}>
              Upgrade → Iniciado
            </Link>
          </div>
        )}
      ...
      <div style={{ padding: 'clamp(28px,4vw,48px) var(--px)' }}>
        ...
        <section id="xp" style={{ marginBottom: 56, scrollMarginTop: 'calc(var(--nav-h) + 8px)' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, borderBottom: '1px solid var(--faint)', paddingBottom: 16, marginBottom: 28 }}>
            ...
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 3, color: 'var(--muted)', textTransform: 'uppercase' }}>Posição no ranking global</p>
          </div>
          ...
          <div className="xp-hero-grid">
            <div style={{ padding: 'clamp(20px,3vw,36px)', borderRight: '1px solid var(--faint)', display: 'flex', flexDirection: 'column', gap: 20 }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 4, color: 'var(--muted)', textTransform: 'uppercase' }}>Rank Atual</p>
              ...
              {nextRank && (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 6 }}>
                    <span>{rank.name}</span>
                  ...
                  <p style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, color: 'var(--muted)', marginTop: 4 }}>
            ...
            <div style={{ padding: 'clamp(20px,3vw,36px)' }}>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 4, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 20 }}>Escala de Ranks</p>
                  ...
                  return (
                    <div key={r.name} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '8px 0', borderBottom: '1px solid var(--faint)' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: isDone ? 'var(--muted)' : isCurrent ? 'var(--gold)' : 'var(--faint)' }}>{r.name}</p>
                        <p style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 1, color: isCurrent ? 'var(--muted)' : 'var(--faint)' }}>{r.min} XP</p>
                      </div>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: isDone ? 'var(--red-dim)' : isCurrent ? 'var(--gold)' : 'transparent' }}>
                        {isDone ? '✓' : isCurrent ? '→' : '○'}
                      </span>
                    </div>
                  )
        ...
        <section id="historico" style={{ marginBottom: 56, scrollMarginTop: 'calc(var(--nav-h) + 8px)' }}>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, borderBottom: '1px solid var(--faint)', paddingBottom: 16, marginBottom: 28 }}>
            ...
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 3, color: 'var(--muted)', textTransform: 'uppercase' }}>Últimas {xpEvents.length} atividades</p>
          </div>
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  {event.transmissoes?.categories?.[0] && (
                    <p className="history-cat" style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 2, color: 'var(--red)', textTransform: 'uppercase', marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <p className="history-date" style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase' }}>
                ...
                <p className="history-xp" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(16px,2vw,22px)', letterSpacing: 2, color: 'var(--gold)', textAlign: 'right' }}>
                  +{event.xp} <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--muted)' }}>XP</span>
                </p>
              </div>
            ))}
            {xpEvents.length === 0 && (
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: 3, color: 'var(--muted)', textTransform: 'uppercase', padding: '32px 0' }}>
                Nenhuma atividade ainda. Comece lendo uma transmissão!
              </p>
            )}
          </div>
        </section>
          ...
          <div className="books-grid">
            {books.map((book: any, i: number) => (
              <div key={book.id} style={{ padding: 'clamp(16px,2vw,24px) clamp(14px,2vw,20px)', borderRight: '1px solid var(--faint)', borderBottom: '1px solid var(--faint)' }}>
                <div style={{ width: '100%', height: 140, background: 'var(--red-faint)', border: '1px solid var(--red-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 8, color: 'var(--red)', letterSpacing: 2, textAlign: 'center', padding: 12, lineHeight: 1.8, marginBottom: 14 }}>
                ...
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 8 }}>{book.author}</p>
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, color: 'var(--red-dim)', textTransform: 'uppercase', marginBottom: 10 }}>{book.month}</p>
                <a href={book.file_url} target="_blank" rel="noopener noreferrer" style={{ display: 'block', width: '100%', padding: 8, textAlign: 'center', fontFamily: 'var(--font-mono)', fontSize: 8, letterSpacing: 2, textTransform: 'uppercase', background: 'transparent', border: '1px solid var(--faint)', color: 'var(--muted)', textDecoration: 'none' }}>
                  Baixar →
                </a>
              </div>
            ))}
        ...
        <div style={{ marginTop: 32, paddingTop: 24, borderTop: '1px solid var(--faint)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          {!profile.is_subscriber && (
            <Link href="/membros" className="btn-primary" style={{ fontSize: 9, padding: '10px 20px' }}>Upgrade → Iniciado</Link>
          )}
          <LogoutButton />
        </div>
...
function LogoutButton() {
  return (
    <form action="/api/auth/logout" method="POST">
      <button type="submit" style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: 3, textTransform: 'uppercase', background: 'transparent', border: '1px solid var(--faint)', color: 'var(--muted)', padding: '10px 20px', cursor: 'pointer', transition: 'all .2s' }}>
        Sair
      </button>
    </form>
  )
