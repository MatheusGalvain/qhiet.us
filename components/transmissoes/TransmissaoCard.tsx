...
export default function TransmissaoCard({ transmissao: t, isSubscriber, labelMap = {} }: TransmissaoCardProps) {
    ...
    <Link href={`/artigo/${t.slug}`} style={{ textDecoration: 'none', display: 'flex', height: '100%' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, letterSpacing: 2, color: t.access === 'free' ? 'var(--red)' : 'var(--gold)', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(36px, 4vw, 52px)', color: isLocked ? 'var(--gold)' : 'rgba(var(--cream), 0.2)', lineHeight: 1 }}>
          {padNumber(t.number)}
        </div>

        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {t.categories.map(cat => {
            const sym = resolveCategorySymbol(cat, labelMap) ?? getCategorySymbol(cat)
            const lbl = resolveCategoryLabel(cat, labelMap)
            return (
